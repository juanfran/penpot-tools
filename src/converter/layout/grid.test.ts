...
describe('gridCellClasses', () => {
  ...
  it('maps alignSelf to self-* class', () => {
    expect(gridCellClasses(makeCell({ alignSelf: 'center' }))).toContain(
      'self-center',
    );
    expect(gridCellClasses(makeCell({ alignSelf: 'start' }))).toContain(
      'self-start',
    );
    expect(gridCellClasses(makeCell({ alignSelf: 'end' }))).toContain(
      'self-end',
    );
    expect(gridCellClasses(makeCell({ alignSelf: 'stretch' }))).toContain(
      'self-stretch',
    );
  });

  it('skips alignSelf when auto or undefined', () => {
    expect(gridCellClasses(makeCell({ alignSelf: 'auto' }))).not.toContain(
      'self-',
    );
  ...
  it('maps justifySelf to justify-self-* class', () => {
    expect(gridCellClasses(makeCell({ justifySelf: 'center' }))).toContain(
      'justify-self-center',
    );
    expect(gridCellClasses(makeCell({ justifySelf: 'start' }))).toContain(
      'justify-self-start',
    );
    expect(gridCellClasses(makeCell({ justifySelf: 'end' }))).toContain(
      'justify-self-end',
    );
    expect(gridCellClasses(makeCell({ justifySelf: 'stretch' }))).toContain(
      'justify-self-stretch',
    );
  });

  it('skips justifySelf when auto or undefined', () => {
    expect(gridCellClasses(makeCell({ justifySelf: 'auto' }))).not.toContain(
      'justify-self-',
    );
