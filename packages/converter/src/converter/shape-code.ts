/**
 * High-level helpers that turn a Penpot shape into framework-ready code:
 *
 *   - `shapeToCode(shape, allObjects, ctx, options)` — the orchestrator
 *     consumers normally call. Composes `convertShape` with class extraction
 *     (CSS or Tailwind), data-attr stripping, and oxfmt formatting.
 *
 *   - The pure transformations underneath (`slugifyName`,
 *     `stylesToCssClasses`, `stylesToTailwind`, `declToTailwind`,
 *     `stripDataAttrs`, `resolveTagOverrides`) are exported individually so
 *     callers with bespoke pipelines (the viewer's live preview, future
 *     codegen tools) can swap pieces.
 *
 ...
 * by this orchestrator and any caller-supplied values for them are ignored.
 */
export async function shapeToCode(
  ...
  const stripped = options.includeDataAttrs ? classed : stripDataAttrs(classed);
  const fileName = options.format === 'jsx' ? 'shape.jsx' : 'shape.html';
  const oxfmt = await import('oxfmt');
  // `htmlWhitespaceSensitivity: 'css'` (oxfmt's default, matching Prettier)
  // treats inline elements like <button>/<p>/<span> as whitespace-sensitive
  // and refuses to add line breaks that would change the rendered output —
  // producing awkward fragments like `</button\n  >`. For exported code we
  // want clean indentation; the agent (or runtime) handles whitespace.
  const { code: formatted } = await oxfmt.format(fileName, stripped, {
 ...
 * Within the same tier, earlier rules win.
 */
 ...
 * with no usable name.
 *
 * Runs in two passes. The first pass walks `<… data-id="…" … style="…">`
 * elements (Penpot shape wrappers) and uses the layer name. The second pass
 * picks up any remaining `style="…"` attributes — text leaves (`<p>`,
 * `<span>`) the converter emits inside `renderParagraph` carry no `data-id`,
 * and would otherwise keep their inline styles after extraction.
 */
