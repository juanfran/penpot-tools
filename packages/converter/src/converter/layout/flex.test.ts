...
describe('flexContainerStyle', () => {
  ...
  it('maps row direction to flex-direction: row', () => {
    expect(flexContainerStyle(makeFrame({ layoutFlexDir: 'row' }))).toContain('flex-direction: row;');
  });

  it('maps column direction to flex-direction: column', () => {
    expect(flexContainerStyle(makeFrame({ layoutFlexDir: 'column' }))).toContain('flex-direction: column;');
  });
  ...
  it('maps alignItems start to align-items: flex-start', () => {
    expect(flexContainerStyle(makeFrame({ layoutAlignItems: 'start' }))).toContain('align-items: flex-start;');
  });

  it('maps alignItems center to align-items: center', () => {
    expect(flexContainerStyle(makeFrame({ layoutAlignItems: 'center' }))).toContain('align-items: center;');
  });

  it('maps alignItems end to align-items: flex-end', () => {
    expect(flexContainerStyle(makeFrame({ layoutAlignItems: 'end' }))).toContain('align-items: flex-end;');
  });

  it('maps alignItems stretch to align-items: stretch', () => {
    expect(flexContainerStyle(makeFrame({ layoutAlignItems: 'stretch' }))).toContain('align-items: stretch;');
  });

  it('maps justifyContent start to justify-content: flex-start', () => {
    expect(flexContainerStyle(makeFrame({ layoutJustifyContent: 'start' }))).toContain('justify-content: flex-start;');
  });

  it('maps justifyContent center to justify-content: center', () => {
    expect(flexContainerStyle(makeFrame({ layoutJustifyContent: 'center' }))).toContain('justify-content: center;');
  });

  it('maps justifyContent end to justify-content: flex-end', () => {
    expect(flexContainerStyle(makeFrame({ layoutJustifyContent: 'end' }))).toContain('justify-content: flex-end;');
  });

  it('maps justifyContent space-between to justify-content: space-between', () => {
    expect(flexContainerStyle(makeFrame({ layoutJustifyContent: 'space-between' }))).toContain('justify-content: space-between;');
  });

  it('maps justifyContent space-around to justify-content: space-around', () => {
    expect(flexContainerStyle(makeFrame({ layoutJustifyContent: 'space-around' }))).toContain('justify-content: space-around;');
  });

  it('maps justifyContent space-evenly to justify-content: space-evenly', () => {
    expect(flexContainerStyle(makeFrame({ layoutJustifyContent: 'space-evenly' }))).toContain('justify-content: space-evenly;');
  });
  ...
  it('maps no-wrap to flex-wrap: nowrap', () => {
    expect(flexContainerStyle(makeFrame({ layoutWrapType: 'no-wrap' }))).toContain('flex-wrap: nowrap;');
  });
...
describe('flexSpacingStyle', () => {
  ...
  it('emits gap: Npx when row and column gap are equal', () => {
    expect(flexSpacingStyle(makeFrame({ layoutRowGap: 8, layoutColumnGap: 8 }))).toContain('gap: 8px;');
  });
  ...
  it('emits padding: Npx when all paddings are equal', () => {
    const result = flexSpacingStyle(makeFrame({ layoutPadding: { p1: 12, p2: 12, p3: 12, p4: 12 } }));
