...
/**
 * Match a single `box-shadow` entry that the converter would emit for an outer
 * stroke: `0 0 0 Npx <color>`. Any other shadow shape (offset, blur, spread
 * with offset) is treated as a real shadow and skipped here.
 */
const OUTER_STROKE_RE =
  /^0(?:px)?\s+0(?:px)?\s+0(?:px)?\s+(-?\d+(?:\.\d+)?)px\s+(.+)$/;
...
export function strokesFromComputed(style: PickedComputedStyle): StrokeExtraction {
  ...
  for (let i = 0; i < shadows.length; i++) {
    const m = shadows[i]!.match(OUTER_STROKE_RE);
    if (!m) continue;
