...
| `get_page_overview`     | _"Show me the page tree; find text/button/image layers"_ — recursive page structure bounded by `maxDepth` and `maxNodes`, with IDs, names, types, compact bounds, text previews, layout flags, and explicit truncation metadata. Optional `query`, `types`, and `scopeId` filters return flat matches; then use `get_html({ shapeId })` for detail.                                                                                                      |
